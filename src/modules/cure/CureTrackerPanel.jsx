...
export default function CureTrackerPanel({ build, onSave }) {
  ...
  const [humidity, setHumidity] = useState(log.humidity);
  const [coats, setCoats] = useState(log.coats || []);

  const product = EPOXY_PRODUCTS.find((p) => p.id === productId) || EPOXY_PRODUCTS[0];
  const adjustedCureHr = adjustCureHours(product.baseCureHr, temperature, humidity);
  const recoatMin = Math.round(product.recoatMin * (humidity > 55 ? 1.15 : 1));
  ...
  const logCoat = () => {
    ...
    if (Notification.permission === "granted") {
      setTimeout(() => {
        new Notification("RodStack Cure Window", { body: `Recoat window open for ${build.buildName || build.name}` });
      }, recoatMin * 60000);
    }
  };
    ...
    <article className="rounded-2xl border border-emerald-500/30 bg-slate-900/80 p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        <label className="text-xs text-slate-400">
          Product
          <select className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-sm" value={productId} onChange={(e) => setProductId(e.target.value)}>
        ...
        <label className="text-xs text-slate-400">
          Temp (°F)
          <input type="number" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-sm" value={temperature} onChange={(e) => setTemperature(Number(e.target.value))} />
        </label>
        <label className="text-xs text-slate-400">
          Humidity (%)
          <input type="number" className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-sm" value={humidity} onChange={(e) => setHumidity(Number(e.target.value))} />
        </label>
      </div>

      <p className="mt-3 text-xs text-slate-500">
        Pot life ~{product.potLifeMin} min · Recoat window ~{recoatMin} min · Adjusted full cure ~{adjustedCureHr} hr
      </p>

      <button type="button" onClick={logCoat} className="mt-3 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">
        Log Coat Applied
      </button>

      <div className="mt-4 space-y-2">
        {coats.length === 0 && <p className="text-sm text-slate-500">No coats logged — apply finish and log each coat for countdown tracking.</p>}
        {coats.map((c, i) => (
          <div key={c.id} className="rounded-lg border border-slate-700 bg-slate-950 p-3 text-xs text-slate-300">
            ...
            <p className="text-slate-500">{c.temperature}°F · {c.humidity}% RH</p>
          </div>
        ))}
      </div>
    </article>
